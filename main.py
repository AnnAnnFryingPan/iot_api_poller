        ...
        for stream in api_streams.streams:
            ...
            if(stream.api_core_url == Data_hub_call_bt.core_URL):
                try:
                    ...
                    cur_params = stream.params

                    bt_hub_response = bt_hub.call_api_fetch(cur_params)

                    if (bt_hub_response.ok):
                        print("BT-Hub call successful. " + str(
                            len(bt_hub_response.content)) + " returned rows from " + stream.users_feed_name)
                        print(bt_hub_response.content)

                        try:
                            influx_db_connection.import_hypercat_response_json(bt_hub_response.content.decode("utf-8"),
                                                                               stream.users_feed_name)
                            print("influxDb call successful: To " + stream.users_feed_name)
                            print(influx_db_connection.query_database(
                                'select value from ' + stream.users_feed_name + ';'))
                        except Exception as err:
                            print("Error populating influx-db: " + str(err))
                    else:
                        print("Error: call to hub: " + stream.api_core_url + " failed with status code: " + \
                               bt_hub_response.reason)

                except Exception as err:
                    print("Error calling Hub: " + stream.api_core_url + ". " + str(err))
                ...
                try:
                    ...
                    if (pi_hub_response.ok):
                        print("Pi-Hub call successful. " + str(
                            len(pi_hub_response.content)) + " returned rows from " + stream.users_feed_name)

                        try:
                            influx_db_connection.import_pi_response_json(pi_hub_response.content.decode("utf-8"),
                                                                         stream.users_feed_name)
                            print("influxDb call successful: To " + stream.users_feed_name)
